...
type PlayerWrap struct {
	...
	OnBattleIDs []*StringKVWrap // 上阵ids
...
func (w *PlayerWrap)Decode(pck *rpc.Packet) rpc.Wrapper {
	...
	w.OnBattleIDs = make([]*StringKVWrap, int(pck.PopInt16()))
	for i := 0; i < len(w.OnBattleIDs); i++ {
		w.OnBattleIDs[i] = new(StringKVWrap)
		w.OnBattleIDs[i].Decode(pck)
...
func (w *PlayerWrap)Encode(pck *rpc.Packet) {
	...
	if w.OnBattleIDs == nil {
		pck.PutInt16(0)
	} else {
    	pck.PutInt16(int16(len(w.OnBattleIDs)))
    	for i := 0; i < len(w.OnBattleIDs); i++ {
    		w.OnBattleIDs[i].Encode(pck);
