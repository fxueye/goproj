...
namespace Game 
{
	...
	{	
		...
		public StringKVWrap[] OnBattleIDs; // 上阵ids
		...
		{	
			...
			OnBattleIDs = new StringKVWrap[pck.GetShort()];
			for (int i = 0; i < OnBattleIDs.Length; i++)
			{
				OnBattleIDs[i] = new StringKVWrap();
				OnBattleIDs[i].Decode(pck);
        ...
        {	
        	...
        	if (OnBattleIDs == null) pck.PutShort((short)0); 
        	else
        	{
	        	pck.PutShort((short)OnBattleIDs.Length);
	        	for(int i = 0; i < OnBattleIDs.Length; i++)
	        	{
	        		OnBattleIDs[i].Encode(pck);
