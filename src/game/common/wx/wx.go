...
func (wx *Weixin) NewLoginPage(newLoginUri string) error {
	...
	if err != nil {
		log.Errorf("parse wxSecret from xml failed: %v", err)
		return err
	}
	if wx.secret.Code == "0" {
...
func (wx *Weixin) GetContacts() error {
	...
	log.Errorf("update %d contacts", r.MemberCount)
...
func (wx *Weixin) updateContacts(us []*User) error {
	...
	if err != nil {
		log.Info("save contacts json encode error:", err)
	}
	err = ioutil.WriteFile("wx-contacts.json", b, 0644)
	if err != nil {
		log.Info("save json write to file error:", err)
	}
	return nil
}
...
func (wx *Weixin) HandleMsg(m *Message) {
	if m.MsgType == 1 { // 文本消息
		log.Errorf("%s: %s", wx.GetUserName(m.FromUserName), m.Content)
		...
		log.Errorf("%s: MsgType: %d", wx.GetUserName(m.FromUserName), m.MsgType)
	}
}
